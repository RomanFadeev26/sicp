...
(define (tagged-list? exp tag)
  (if (pair? exp)
      (eq? (car exp) tag)
      false))
(define (index-of var items)
  (define (inner count items)
    (cond ((null? items) false)
          ((eq? var (car items)) count)
          (else (inner (+ count 1) (cdr items)))))
  (inner 0 items))
...
(define the-empty-ct-env '())
(define (extend-ct-env vars base-env)
  (cons vars base-env))
...
(define (get-frame-by-address env address)
  (list-ref env address))

(define (get-value-by-displacement values displacement)
  (list-ref values displacement))

(define (set-value-by-index values index new-value)
  (set-car! (list-tail values index) new-value))

(define (lexical-address-lookup env lexical-address)
  (let ((number-of-frame (frame-number lexical-address))
        (number-of-displacement (displacement-number lexical-address)))
    (let ((frame (get-frame-by-address env number-of-frame)))
      (let ((val (get-value-by-displacement (frame-values frame) number-of-displacement)))
        (if (eq? val '*unassigned*)
            (error  "Нет значения переменной по адресу -- LEXICAL-ADDRESS-LOOKUP" lexical-address)
            val)))))

(define (lexical-address-set! env lexical-address new-value)
  (let ((number-of-frame (frame-number lexical-address))
        (number-of-displacement (displacement-number lexical-address)))
    (let ((frame (get-frame-by-address env number-of-frame)))
      (set-value-by-index (frame-values frame) number-of-displacement new-value))))

(define (find-variable var ct-env)
    (define (find-var frame-addr ct-env)
      (if (null? ct-env)
          'not-found    
          (let ((first-frame (car ct-env)))
            (let ((index (index-of var first-frame)))
              (if index
                  (cons frame-addr index)
                  (find-var (+ 1 frame-addr) (cdr ct-env)))))))
      (find-var 0 ct-env))
     ...
     (make-instruction-sequence '(env proc argl) '(env)
                                `(,proc-entry
                                  ...
                                  (assign env
                                          ...
                                          (reg argl)
                                          (reg env))))
     (compile-sequence (lambda-body exp) 'val 'return (extend-ct-env formals ct-env)))))
